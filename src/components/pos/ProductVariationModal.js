...
const ProductVariationModal = ({
  ...
  const checkVariantAvailability = (productData) => {
    productData?.variations?.forEach((variation) => {
      variation?.attribute_values?.forEach((attribute_value) => {
        if (
          cloneVariation.attributeValues?.indexOf(attribute_value?.id) === -1
        ) {
  ...
  const wholesalePriceCal = () => {
    let wholesale = cloneVariation?.product?.wholesales?.length
      ? cloneVariation?.product?.wholesales?.find(
          (value) =>
            value.min_qty <= cloneVariation.productQty &&
            value.max_qty >= cloneVariation.productQty
        )
      : null;
    ...
    if (wholesale && cloneVariation?.product.wholesale_price_type == "fixed") {
      ...
      wholesale &&
      cloneVariation?.product.wholesale_price_type == "percentage"
    ) {
      setCloneVariation((prev) => {
        return {
          ...prev,
      ...
      <Col lg="6">
        <div className="right-sidebar-modal">
          ...
          <div className="modal-bottom-cart">
            <VariationModalQty
              wholesalePriceCal={wholesalePriceCal}
              cloneVariation={cloneVariation}
